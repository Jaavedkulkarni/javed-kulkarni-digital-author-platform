...
/** Returns whether the current user has any of the required roles. */
export function useHasAnyRole(requiredRoles: SystemRole[]): MultiRolePredicateState {
  const { roles, loading } = useAuthRoles();

  return useMemo(() => {
    const resolver = createRoleResolver(roles);
    return {
      satisfied: resolver.hasAnyRole(requiredRoles),
      isLoading: loading,
    };
