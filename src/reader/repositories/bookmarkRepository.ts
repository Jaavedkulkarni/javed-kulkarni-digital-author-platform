...
export class ReaderBookmarkRepository extends BaseRepository<'bookmarks'> {
  constructor(client: TypedSupabaseClient) {
    super(client, 'bookmarks', { softDelete: true });
  }

  async findByUser(userId: string, bookId?: string): Promise<Tables<'bookmarks'>[]> {
...
export function createReaderBookmarkRepository(client: TypedSupabaseClient): ReaderBookmarkRepository {
  return new ReaderBookmarkRepository(client);
}
